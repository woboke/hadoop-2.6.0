...
public class FSImage implements Closeable {
  ...
  //声明布尔类型--是否升级完成 默认为false
  private boolean isUpgradeFinalized = false;
  //NNStorage负责管理NameNode所使用的存储目录
  protected NNStorage storage;
  
  /**
   * The last transaction ID that was either loaded from an image
   * or loaded by loading edits files.
   * 最后一个transaction ID(事务id) 要么从镜像中加载 要么从编辑的文件(fseditlog)中加载
   */
   ...
   * and writing it out.
   * FSImageSaver在保存FSImage时正在单独的线程中运行。每个图像副本都有一个线程。
   * FSImageSaver假定它是从保存FSNamesystem锁的线程启动的，
   * 并等待FSImageSaver线程执行完成。通过这种方式，我们可以保证在FSImageSaver的多个实例遍历并写出时名称空间不会被更新。
   *
   * 实现Runnable接口 实现多线程
   */
